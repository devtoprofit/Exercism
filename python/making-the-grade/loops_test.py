...
class MakingTheGradeTest(unittest.TestCase):
    ...
    def test_round_scores(self) -> None:
        ...
        for variant, (student_scores, expected) in enumerate(
            ...
            with self.subTest(f'variation #{variant}',
                              student_scores=student_scores, expected=expected):

                # Because the test_input is a tuple, it has to be converted to a list
    ...
    def test_above_threshold(self) -> None:
        test_data: list[tuple[list[int], int]] = [([40, 39, 95, 80, 25, 31, 70, 55, 40, 90], 98),
    ...
    def test_perfect_score(self) -> None:
        test_data: list[list[list[str | int]]] = [
        ...
        result_data: list[list[str | int]] = [['Joci', 100],[], [], [], ['Raiana', 100]]

        for variant, (student_info, expected) in enumerate(
            zip(test_data, result_data, strict=True), start=1):

            with self.subTest(f'variation #{variant}',
                              student_info=student_info, expected=expected):
