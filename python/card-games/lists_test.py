...
class CardGamesTest(unittest.TestCase):
    ...
    def test_concatenate_rounds(self) -> None:
        ...
        input_data: list[tuple[list[int], ...]] = [([], []), ([0, 1], []), ([], [1, 2]),
                      ([1], [2]), ([27, 28, 29], [35, 36]),
                      ([1, 2, 3], [4, 5, 6])]

        result_data: list[list[int]] = [[], [0, 1], [1, 2], [1, 2],
                       [27, 28, 29, 35, 36],
                       [1, 2, 3, 4, 5, 6]]

        for variant, ((rounds_1, rounds_2), expected) in enumerate(
            zip(input_data, result_data, strict=True), start=1):
            with self.subTest(f'variation #{variant}', rounds_1=rounds_1,
                              rounds_2=rounds_2, expected=expected):
    ...
    def test_list_contains_round(self) -> None:
        ...
        input_data: list[tuple[list[int], int]] = [([], 1), ([1, 2, 3], 0),
                      ([27, 28, 29, 35, 36], 30),
                      ([1], 1), ([1, 2, 3], 1),
                      ([27, 28, 29, 35, 36], 29)]
        ...
        for variant, ((rounds, round_number), expected) in enumerate(
            zip(input_data, result_data, strict=True), start=1):
            with self.subTest(f'variation #{variant}', rounds=rounds,
                              round_number=round_number, expected=expected):
